...
export default class Signal extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.socket.on('open', () => {
      ...
      console.log("sfuconnect on open. now trying messageQueue processing.");
      if (this.messageQueue.queue && this.messageQueue.queue.length>0) {
        this.socket.send(this.messageQueue.queue[0])
  ...
  send(object){
    ...
    if (this?.socket?.rawSocket?.readyState === WebSocket.OPEN) {
      console.log("WEBSOCKET IS OPEN")
      this.socket.send(object);
    } else {
      console.log("WEBSOCKET NOT OPEN. Pushing message in queue.");
